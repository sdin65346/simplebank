...
func (store *SQLStore) execTx(ctx context.Context, fn func(*Queries) error) error {
	tx, err := store.db.BeginTx(ctx, nil) //начинаем новую транзакцию
	if err != nil {
		return nil
	}

	//вызываем функцию с созданной транзакцией и получааем Queries
	//передаем sql.Tx
	q := New(tx)
	err = fn(q)
	if err != nil {
...
func addMoney(
	...
	if err != nil {
		return //equal return account1, account2, err
	}
